...
class LLMJudge:
    ...
    def score_response(
        ...
        default_scores = {str(k): 0.5 for k in rubric.keys()}
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_f = sum(r.faithfulness for r in new_results) / n_total if n_total > 0 else 0.0
        new_r = sum(r.relevance for r in new_results) / n_total if n_total > 0 else 0.0
        new_c = sum(r.completeness for r in new_results) / n_total if n_total > 0 else 0.0

        base_f = sum(r.faithfulness for r in baseline_results) / b_total if b_total > 0 else 0.0
        base_r = sum(r.relevance for r in baseline_results) / b_total if b_total > 0 else 0.0
        base_c = sum(r.completeness for r in baseline_results) / b_total if b_total > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if failure.faithfulness == min_score and failure.faithfulness < 0.5:
            return "Context is missing or irrelevant — improve retrieval"
        elif failure.relevance == min_score and failure.relevance < 0.5:
            return "Answer does not address the question — improve prompt clarity"
        elif failure.completeness == min_score and failure.completeness < 0.5:
            return (
                "Answer is missing key information — increase context window or improve generation"
            )
        else:
    ...
    def generate_improvement_suggestions(
        ...
        if not failures:
            return [
                ...
                "Expand golden dataset to include edge cases.",
                "Maintain regression testing quality gate.",
            ]
        ...
        if (
            categories.get("incomplete", 0) > 0
            or categories.get("Low_completeness", 0) > 0
            or categories.get("Incomplete", 0) > 0
        ):
            suggestions.append(
                "Increase chunk size in RAG pipeline to reduce context fragmentation"
            )
        ...
        if categories.get("irrelevant", 0) > 0 or categories.get("Irrelevant", 0) > 0:
            suggestions.append(
                "Improve prompt clarity and intent classification to handle question nuance"
            )
